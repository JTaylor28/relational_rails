...
RSpec.describe 'concert show page' do 
  it 'displays band and band attributes' do
    ...
    band = Band.create(band_name: "Chon", headliner: false, members: 5, concerts_id: concert.id )
    band2 = Band.create(band_name: "polyphia", headliner: true, members: 4, concerts_id: concert2.id )
    ...
    expect(page).to have_content(band.concerts_id)
    ...
    expect(page).to_not have_content(band2.concerts_id)
    ...
    expect(page).to have_content(band2.concerts_id)
    ...
    expect(page).to_not have_content(band.concerts_id)
